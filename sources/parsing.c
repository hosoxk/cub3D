...
int	parse_configuration(t_map *map, char **lines, int line_count)
{
	...
	while (i < line_count)
	{
		...
		else if (is_config_line(trimmed))
		{
			...
			return (print_error("Error: unrecognized line in .cub file"), -1);
		i++;
	}
	if (!map_started)
